<x-app-layout>
    ...
    <div class="d-flex justify-content-center">
        <div class="container-md">
            <h2>Gastos: editar</h2> <br>
        
            <form action="{{ route('expenses.update', [$expense->id]) }}" method="post" class="app-form">
                <input type="hidden" name="_method" value="PUT">
                @csrf

                <div class="mb-3">
                    <label for="dateInput" class="form-label">Fecha</label>
                    <input type="text" name="date" class="form-control form-control-lg app-datepicker" id="dateInput" value="{{ old('date', $expense->date) }}" autocomplete="off">
                ...
                <div class="mb-3">
                    ...
                    <select name="unit_id" id="unitInput" class="form-select form-select-lg">
                        ...
                        @foreach($units as $unit)
                            <option 
                                {{ $unit->id == old('unit_id', $expense->unit_id) ? 'selected' : '' }}
                                value="{{ $unit->id }}">
                                {{ $unit->name }}
                            </option>
                        @endforeach
                ...
                <div class="mb-3">
                    ...
                    <select name="supplier_id" id="supplierInput" class="form-select form-select-lg">
                        ...
                        @foreach($suppliers as $supplier)
                            <option 
                                {{ $supplier->id == old('supplier_id', $expense->supplier_id) ? 'selected' : '' }}
                                value="{{ $supplier->id }}">
                                {{ $supplier->name }}
                            </option>
                        @endforeach
                ...
                <div class="mb-3">
                    ...
                    <select name="expenses_category_id" id="expensesCategoryInput" class="form-select form-select-lg">
                        ...
                        @foreach($expenses_categories as $expenses_category)
                            <option 
                                {{ $expenses_category->id == old('expenses_category_id', $expense->expenses_category_id) ? 'selected' : '' }}
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" name="description" id="descriptionInput" rows="5">{{ old('description', $expense->description) }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="amount" class="form-control form-control-lg" id="amountInput" value="{{ old('amount', $expense->amount) }}">
                ...
                <div class="form-check">
                    ...
                    <input class="form-check-input" type="checkbox" value="1" name="billed" id="billedInput" {{ old('billed', $expense->billed) ? 'checked' : '' }}>
